    ...
    SemanticLabel*           semantic_label)
{
    ...
    switch (object_type) {
        ...
        default: LOG(FATAL) << "Object type not recognized.";
    }
}
    ...
    vxb::Layer<SemanticVoxel>* layer)
{
    ...
    for (vxb::FloatingPoint x = min_bound.x() - half_block_size;
         x <= max_bound.x() + half_block_size; x += block_size) {
        for (vxb::FloatingPoint y = min_bound.y() - half_block_size;
             y <= max_bound.y() + half_block_size; y += block_size) {
            for (vxb::FloatingPoint z = min_bound.z() - half_block_size;
                 z <= max_bound.z() + half_block_size; z += block_size) {
                blocks.push_back(layer->computeBlockIndexFromCoordinates(
                    vxb::Point(x, y, z)));
            }
        }
    }
    ...
    for (const vxb::BlockIndex& block_index : blocks) {
        ...
        for (size_t i = 0; i < block->num_voxels(); ++i) {
            ...
            for (const std::unique_ptr<vxb::Object>& object : objects_) {
                vxb::FloatingPoint object_dist =
                    object->getDistanceToPoint(coords);
