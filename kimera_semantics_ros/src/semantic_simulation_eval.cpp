...
class SemanticSimulationServerImpl : public SemanticSimulationServer {
  public:
    ...
    {
        CHECK(world_);
        world_->addObject(make_unique<vxb::Sphere>(vxb::Point(0.0, 0.0, 2.0),
                                                   2.0, vxb::Color::Red()));

        world_->addObject(make_unique<vxb::PlaneObject>(
            vxb::Point(-2.0, -4.0, 2.0), vxb::Point(0, 1, 0),
            vxb::Color::White()));

        world_->addObject(make_unique<vxb::PlaneObject>(
            vxb::Point(4.0, 0.0, 0.0), vxb::Point(-1, 0, 0),
            vxb::Color::Pink()));

        world_->addObject(make_unique<vxb::Cube>(vxb::Point(-4.0, 4.0, 2.0),
                                                 vxb::Point(4, 4, 4),
                                                 vxb::Color::Green()));
