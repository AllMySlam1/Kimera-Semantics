      ...
      semantic_config_(getSemanticTsdfIntegratorConfigFromRosParam(nh_private))
{
    ...
    tsdf_integrator_ = SemanticTsdfIntegratorFactory::create(
        SemanticTsdfIntegratorType::kFast, tsdf_integrator_->getConfig(),
        semantic_config_, tsdf_test_.get(), semantic_test_.get());
...
bool SemanticSimulationServer::loadTsdfMap(const std::string& file_path)
{
    ...
    bool           success               = vxb::io::LoadBlocksFromFile(
        file_path, vxb::Layer<vxb::TsdfVoxel>::BlockMergingStrategy::kReplace,
        kMulitpleLayerSupport, tsdf_test_.get());
    if (success) {
...
bool SemanticSimulationServer::loadEsdfMap(const std::string& file_path)
{
    ...
    constexpr bool kMulitpleLayerSupport = true;
    bool           success               = vxb::io::LoadBlocksFromFile(
        file_path, vxb::Layer<vxb::EsdfVoxel>::BlockMergingStrategy::kReplace,
        kMulitpleLayerSupport, esdf_test_.get());
    if (success) {
