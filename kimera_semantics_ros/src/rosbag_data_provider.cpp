...
RosbagDataProvider::RosbagDataProvider()
    : nh_(), nh_private_("~"), rosbag_data_(nullptr), rosbag_path_(""),
      depth_imgs_topic_(""), semantic_imgs_topic_(""), rgb_imgs_topic_(""),
      left_cam_info_topic_(""), clock_pub_(), depth_img_pub_(),
      semantic_img_pub_(), rgb_img_pub_(),
...
bool RosbagDataProvider::parseRosbag(const std::string& bag_path)
{
    ...
    for (const rosbag::MessageInstance& msg : view) {
        if (!nh_.ok() || !ros::ok() || ros::isShuttingDown())
            return false;
        LOG(INFO) << "Rosbag processing: "
                  << msg.getTime() - view.getBeginTime() << " / "
                  << rosbag_duration;
        ...
        if (tf_msg != nullptr) {
            ...
            for (const geometry_msgs::TransformStamped& transform_stamped :
                ...
                // It may be the case that we only have base_link to
                // world_frame, and then we need to apply tf from base_link to
                // depth_cam...
    ...
    LOG_IF(FATAL, rosbag_data_->depth_imgs_.size() !=
                      rosbag_data_->semantic_imgs_.size())
...
void RosbagDataProvider::publishInputs(const Timestamp& timestamp_kf)
{
    ...
    if (k_last_kf_ < rosbag_data_->depth_imgs_.size() &&
        ...
        while (timestamp_last_kf_ < timestamp_kf &&
            ...
            semantic_img_pub_.publish(
                rosbag_data_->semantic_imgs_.at(k_last_kf_));
